...
EmuDiscer::EmuDiscer(QSharedMemory *sharedMem, QWidget *parent)
    : QDialog(parent), mConfig((getConfigDirectory() + "/config.ini").toStdWString(), true),
      mSharedMem(sharedMem)
{
    ...
    connect(mSysTrayIcon, SIGNAL(activated(QSystemTrayIcon::ActivationReason)), this,
            SLOT(on_trayIconActivated(QSystemTrayIcon::ActivationReason)));
    ...
    connect(qApp, SIGNAL(saveStateRequest(QSessionManager &)), this,
            SLOT(on_saveStateRequest(QSessionManager &)));
    mLineeditDefaultPalette = ui.pathEdit->palette();

    /*Macro list*/
    mMacrosMenu = new QMenu(this);
    for (QString macro : {"(DRIVE)", "(DIRECTORY)", "(BOOT_FILE)"}) {
    ...
    connect(udisks2watcher, SIGNAL(partitionChanged(QString)), this,
            SLOT(on_partitionChanged(QString)));
    connect(udisks2watcher, SIGNAL(driveMounted(QString, QString)), this,
            SLOT(on_driveMounted(QString, QString)));
#endif
    ...
    */
    ...
    ui.autoRunCheckbox->setCheckState(isAutoRun() ? Qt::CheckState::Checked
                                                  : Qt::CheckState::Unchecked);
    ui.autoRunCheckbox->blockSignals(false);
    ui.notificationsCheckbox->setCheckState(mConfig.get<bool>(L"System", L"ShowNotifications", true)
...
bool EmuDiscer::nativeEvent(const QByteArray &eventType, void *message, qintptr *result)
{
#ifdef WIN32
    MSG *msg = (MSG *)message;
        ...
        if (msg->wParam == DBT_DEVICEARRIVAL) {
            if (((PDEV_BROADCAST_HDR)msg->lParam)->dbch_devicetype == DBT_DEVTYP_VOLUME) {
                ...
                // if (pDBV->dbcv_flags & DBTF_MEDIA)
                {
                    ...
                    charsToRead =
                        getDriveFileHeader(QString(driveLetter), headerBuffer, charsToRead);

                    // launch Emu
                    if (insertedMediaDir(QString() + driveLetter + ":", QString(driveLetter))) {
                    } else if (insertedMediaRaw(QString(driveLetter), headerBuffer, charsToRead)) {
                    }
                }
            }
        }
...
bool EmuDiscer::tryLaunchEmu(QString emulatorType, QString mediaType,
                             const std::map<std::wstring, std::wstring> &vars)
{
    if (emulatorType != "") {
        ...
        if (path != L"" && fileExists(QString::fromStdWString(path))) {
            ...
            startProgram(QString::fromStdWString(path), QString::fromStdWString(options),
                         QString::fromStdWString(L""), {});
        } else if (mConfig.get<bool>(L"System", L"ShowNotifications")) {
            mLastEmulatorType = emulatorType;
            mSysTrayIcon->showMessage("No emulator",
                                      QString("A %1 has been inserted but a %2 emulator has not "
                                              "been set up. Click to set it up.")
                                          .arg(mediaType, emulatorType),
                                      windowIcon());
        }
...
bool EmuDiscer::insertedMediaRaw(QString drive, const char *header, size_t headerSize)
{
    ...
    if (headerSize >= 32 && memcmp(header + 0x1b, "\x00\xC2\x33\x9F\x3D", 5) == 0) {
    ...
    // Wii (the bytes between 17 and 1F seem to always be the 00 5D 1C 9E A3 00 00 00 00)
             ...
             memcmp(header + 0x17, "\x00\x5D\x1C\x9E\xA3\x00\x00\x00\x00", 9) == 0) {
...
void EmuDiscer::on_emulatorsTabWidget_currentChanged(int index)
{
    ...
    ui.optionsEdit->setText(QString::fromStdWString(
        mConfig.getStr(mSelectedEmulator, L"Options"))); // this goes before pathEdit
...
void EmuDiscer::on_pathButton_clicked()
{
    ...
    QFileDialog dialog;
    QString filename =
        dialog.getOpenFileName(this, "Select executable", ui.pathEdit->text(), getExeFileFilter());
