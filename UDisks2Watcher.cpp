...
QStringList aay2StringList(const QVariant &aayContainingVariant)
{
    ...
    // The argument type is 'aay' which seems too complex for Qt to proccess, so we *simply* extract
    // it fml
    ...
    if (aayContainingVariant.canConvert<QDBusArgument>()) {
        ...
        throw std::runtime_error(std::string("Can't retrieve QDBusArgument: variant is ") +
...
UDisks2Watcher::UDisks2Watcher(QObject *parent) : QObject(parent)
{
    ...
    QDBusConnection::systemBus().connect(ServiceName, PathPrefix,
...
void UDisks2Watcher::updateInterfaceList()
{
    ...
    QDBusMessage reply = QDBusConnection::systemBus().call(QDBusMessage::createMethodCall(
        ServiceName, PathPrefix, ObjectManagerInterface, "GetManagedObjects"));
...
void UDisks2Watcher::on_interfaceAdded(QString objPath)
{
    ...
    if (objPath.startsWith(PathPrefix + "/block_devices")) {
        ...
        if (mPathWatcherMap.find(objPath) == mPathWatcherMap.end()) {
            ...
            connect(mPathWatcherMap[objPath].get(),
                    SIGNAL(objectPropertiesChanged(QString, QVariantMap)), this,
                    SLOT(on_blockDevicePropertiesChanged(QString, QVariantMap)));
        }

        // check mount points
        QDBusMessage reply = QDBusConnection::systemBus().call(
            QDBusMessage::createMethodCall(ServiceName, objPath, PropertiesInterface, "Get")
...
void UDisks2Watcher::on_interfaceRemoved(QString objPath)
{
    if (objPath.startsWith(PathPrefix + "/block_devices")) {
        ...
        if (it != mPathWatcherMap.end()) {
            disconnect(it->second.get(), SIGNAL(objectPropertiesChanged(QString, QVariantMap)),
...
UDisks2PropertyChangeWatcher::UDisks2PropertyChangeWatcher(QString objPath, QObject *parent)
    : QObject(parent), mObjPath(objPath)
{
    QDBusConnection::systemBus().connect(ServiceName, mObjPath, "org.freedesktop.DBus.Properties",
                                         "PropertiesChanged", this,
                                         SLOT(on_propertiesChanged(QString, QVariantMap)));
}

UDisks2PropertyChangeWatcher::~UDisks2PropertyChangeWatcher()
{
    QDBusConnection::systemBus().disconnect(
        ServiceName, mObjPath, "org.freedesktop.DBus.Properties", "PropertiesChanged", this,
        SLOT(on_blockDevicePropertiesChanged(QString, QVariantMap)));
}
